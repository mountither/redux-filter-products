...
const filterInfo = [{
  ...
  const input = new normalizr__WEBPACK_IMPORTED_MODULE_0__["schema"].Entity('input', undefined, {
    idAttribute: (entity, parent) => `${parent.id}.${entity.id}`
  });
  const filter = new normalizr__WEBPACK_IMPORTED_MODULE_0__["schema"].Entity('filter', {
    input: [input] // field:field
  
  });
  const normalizedData = Object(normalizr__WEBPACK_IMPORTED_MODULE_0__["normalize"])(filterInfo, [filter]);
